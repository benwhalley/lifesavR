...
`mtcars`. This is a dataset about cars that was published in a US magazine called Motor Trend. As we did with the
`diamonds` tibble, if we type `mtcars`, select the variable name and execute it, we can see the data it contains.
...
:::{.exercise}
1. After the line of code your wrote in Exercise 1, display the `mtcars` data.frame and try out the navigation buttons
