...
`r hide('Video script')`
Colour can be added to a scatterplot to categorise the points. The command you've been using to define which variable is plotted on the x and y axes is `aes`. The `aes` command is short for 'aesthetics'. It maps variables to visual aspects of a plot. In addition to mapping variables to the x and y axes, adding a `colour` option to `aes` maps colours to a categorical variable.
...
1. Create a new chunk at the bottom of your worksheet
1. Create a scatterplot using the `gapminder` dataset `gdpPercap` on the x-axis, `lifeExp` on the y-axis, and `continent` in colour.
1. Run the chunk.
