...
In each session you will work in a single R Markdown file. Click on `session-1.rmd`, which is the file you need for this
session.

## Running R code in a chunk

There are two ways to run code within a chunk:

1. Run all of the code
1. Run a line of code at a time

We'll start with the second method:

:::{.exercise}
1. Locate the first chunk in `session-1.rmd`.
1. Place your cursor (anywhere) on the line that says `library(tidyverse)`
1. Run this line by pressing `Ctrl+enter` (`command+enter` on a Mac).
...
Now position your cursor on the line that says `diamonds` and press `Ctrl+enter`. You should see the a scatterplot of the
diamonds data appear below the chunk:
:::
...
Congratulations! You have just run your first lines of R.

The code you ran to generate the scatterplot consisted of three lines. You can also run a subset of this command:

:::{.exercise}
1. Select (highlight) the word `diamonds` and then press `Ctrl+enter`.
...
Why would you want to run part of a command? In these workshops you will combine simple steps into sequences which do a
particular job, such as generating a plot. It's natural, especially when you're new to R, that the full sequence won't
do exactly what you want. Running part of a command allows you to identify the steps which are correct so that you can
modify subsequent steps until your code produces the required results. Remember this technique as you will be use it
extensively in these workshops.
