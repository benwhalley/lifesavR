...
Press <span class="keyshape">↩</span> to run the command. If your console looks like the image below, then you are ready to start the session.
...
do a particular job, such as generating a plot. It's natural, especially when you're new to R, that the full sequence of
commands won't do exactly what you want first time.
...
- Loading a 'package' adds commands to R.
...
`r hide('Video script')`
By loading 'packages', you can add commands and datasets to R. Packages are a powerful feature which allow R to be
...
<!-- TODO "can we find a simpler dataset, or perhaps do this after you introduce loading a dataset over the web then we could provide a short link to a cut-down version of it?" -->

The [World Happiness Report](https://www.kaggle.com/ajaypalsinghlo/world-happiness-report-2021) is a survey of the state
of global happiness. Running the command `happiness %>% glimpse()` shows all of the columns. This is like rotating the
output you saw earlier anti-clockwise by 90 degrees. The command displays all of the columns as rows, and as many
...
- A scatterplot shows the relationship between two variables by plotting their values as points. The `tidyverse` package
also loads the `mpg` dataset, which contains fuel economy data from 1999 to 2008 for 38 popular models of US cars.
...
1. How do you run part of an R command?
1. Which library do you need to load in your first R Markdown chunk?
1. What is `gapminder`?
1. Which command would you use to explore the first few rows of a dataset?
1. What is the 5th column in `gapminder`?
1. Explain what `glimpse` does.
1. Which command makes a plot?
1. Which command defines the axes on a plot?
1. Which command makes a boxplot?
