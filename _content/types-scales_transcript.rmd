...
This gives us the boxplot we were expecting. The only change was to replace `cyl` with `factor(cyl)`. This tells R to convert the variable `cyl` to a factor, and  `ggplot` can then set the scale of the x axis correctly.
