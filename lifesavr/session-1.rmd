...
- Working interactively in Rmarkdown
...
University](https://rstudio.plymouth.ac.uk), but they should be easy to follow if you are using the desktop version of
RStudio.
...
Before starting this module, you need to run an `R` command which:

- Creates a directory called `lifesavr` in your `Home` directory

- Installs some R Markdown files. These have the file extension `.rmd`. You will store your work in one of these files in each session. For example, in session one you will only need to edit `session-1.rmd`.

- Downloads the data used in the module into the `lifesavr/data` folder.
...
2. Press return (enter) to run the command. Your console should now look something like this:

![](images/bootstrap_success.png)

:::{.exercise}

Check that you have all of the files listed above by

1. Using the **Files** pane to open the `lifesavr` folder
1. Comparing the files in the `lifesavr` folder with the image below.

![](images/bootstrap_files.png)
...
At this point check the Environment window again to see it there
