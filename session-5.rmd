...
XXX TODO PAUL _ ADD WORKBOOK PARTS AND REMINDERS HERE

- Use the `funimagery` dataset
- Plot participants' weights at baseline (`kg1`) in each intervention group
- Use `summarise()` to calculate the difference between the groups weight
- Use `with()` and `ttestBF()` to calculate the Bayes Factor for this difference
