...
Only some of these possible red lines are shown in the plot — but the correlation can be anything between -1 and 1. The test is saying, *"given the data we have collected, how much more likely is it that the slope is non-zero, than zero?"*
...
Let's say we wanted the Bayes Factor for the correlation between engine size and power in the `fuel` data set.
...
```{r, echo=F}
fuel %>%
  ggplot(aes(engine_size, power)) +
  geom_point()
```


It seems like it is. The plot also suggests a positive correlation. We can check this with:

```{r}
diamonds %>% select(carat, price) %>% correlate()
```
