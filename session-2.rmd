...
<!-- TODO Paul can you clarify — is the data to be used the shooting or school data? Perhaps introducing 2 different files 
...
- The `arrange()` command sorts rows in a dataset.
...
- Use commas between column names to sort by one column within another.
...
`r hide('Video script')`
<!-- TODO: Ben, wasn't sure why/where to "remind them they know how to make scatter and boxplots" -->
...
- Pipelines often combine `filter()` and `arrange()` to answer specific questions.
...
1. Sort Asian countries in the `gapminder` dataset in ascending population order.
...
To two decimal places, cars with automatic transmission have a standard deviation of `r fitb('3.83')` miles per gallon.
:::
...
- Answer which requires sorting by 2 columns in different order.
