...
In R, a **variable** is the name for a *container* which *stores data*. We make variables using `<-`, which is called the
**assignment operator**.
...
That is, like an arrow that points to the left. This is a reminder that the results of the
calculation on the right hand side will be assigned (stored) in the variable on the left hand side. 
...
1. Use R to calculate your age in the year 2051. Save the result in a variable. Give your variable a descriptive name.
...
<!-- TODO Paul - what are the square brackets below supposed to indciate? -->
...
You can think of your data as flowing along lengths of pipe, joined by commands which do things to the data, step by step,
until the result you want plops out at the end. 

Each command should be read as the word "then", e.g. "pipe `mtcars` data,
*then* `head` it". 
...
<!-- TODO ARE SQ BRACKETS SUPPOSED TO BE AN INTERNAL LINK -->
...
1. Create a level 3 markdown heading named "Exercise 3" in your workbook. (You should be used to doing this for every exercise by now, so we won't remind you again.)  <!-- TODO is this true enough yet? -->
1. Create a new chunk beneath the heading
...
- Data in CSV format can be read using the `read_csv` command.
- You can load data either from an internet address (URL) or from a file uploaded to the RStudio server
...
<!-- TODO can you find a link to Andy's 'why csv?' explanation? -->


If you click on the link you'll see the first line is a list of column names separated by commas. 
...
An alternative (and recommnded) way is to type the name of the variable as a very simple command:
...
The `Upload` button in the **Files** pane lets you upload a data file from your computer to R Studio, so you can read it
using `read_csv()`.
...
- `filter()` selects rows from a dataset which match the criteria we set
