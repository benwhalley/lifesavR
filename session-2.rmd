...
`r hide('Video script')`
The file [https://benwhalley.github.io/lifesavR/lifesavr/shootings.csv] is a CSV file containing
data about US police shootings. The first line is a list of column names separated by commas. The remaining lines
contain rows of data matching the column headings. For example, the value of the `arms_category` column in row 1 is
`Guns`.
...
The `filter()` command returns a subset of rows from a data.frame based on column values. The simplest way to filter
data is using `==`.
