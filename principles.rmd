...
```{r foo}
personality <- read_csv('lifesavr/data/2018-personality-data.csv')
ratings <- read_csv('lifesavr/data/2018_ratings.csv') %>% rename(userid=useri)
...
```{r}
read_csv('lifesavr/data/shootings.csv') %>% 
  group_by(race, flee) %>% 
  count() %>% 
...
```{r schools, eval=F}
schools <- read_csv('lifesavr/data/schoolpredict.csv')
