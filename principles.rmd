...
GENERAL PRINCIPLE - use existing explanations/code from VBGR where possible (and don't do things a different way unless for good reason)
...
```{r}
personality <- read_csv('~/Downloads/archive/2018-personality-data.csv')
ratings <- read_csv('~/Downloads/archive/2018_ratings.csv') %>% rename(userid=useri)
...
```{r}
read_csv('~/Downloads/shootings.csv') %>% 
  group_by(race, flee) %>% 
  count() %>% 
...
```{r}
schools <- read_csv('~/Downloads/schoolpredict.csv')
...
schools %>% 
  ...
  ggplot(aes(PNORB, GPS_AVERAGE, color=school_sex)) + geom_jitter()
...
Code to fix will be simila to that shown in the demo video 
