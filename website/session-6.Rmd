...
```{r, echo=F}
augment(simple1, newdata = tibble(screen_time = c(2, 24))) %>% 
  pull(.fitted) %>% round(3) %>% 
  pandoc.list()
```
...
- Residuals can be calculated which which R function? `r mcq(whichrresdid)`
...
have 32 datapoints in the sample. Likewise, it's hard to tell from the histogram if the resduals are normal.
However the fitted vs residual plot does hint that residuals are not evenly-sized across the range of the 
fitted values, so it might be worth exploring further (see next section).
...
In an [example above](#violationexample) we saw that a model predicting life expectancy from GDP per-person lead us to violate some assumptions of regression. There are many different techniques which can be used to work around this kind of problem. Here we illustrate just one.
