...
```{r, echo=F}
video_data <- list(identifier = "variables", yt_id = "rZbJrccgk80")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "pipes", yt_id="APFXK-xoxR0")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "loading-data", yt_id = "N78Z98vs9vQ")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "filter", yt_id = "9N8sRAoFYzc")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "summarise", yt_id="ZoCJTBdfVwQ")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "fixing-code", yt_id = "NByWIgA9LA0", showcode = T)
#makermds(video_data)
```
...
The mean petal width of the versicolour species of iris is `r fitb(correct, tol=.2)`cm.
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/qIS4UILV26c" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
