...
and used functions to select and summarise subsets of the data. These are important building blocks for later sessions.
...
It looks like an arrow that points to the left, but it's made up of a left angle bracket (`<`) and a hyphen (`-`),
...
In the video we run the line with variable assignment.
:::

And we now have a variable called `current_year` which contains the number `2026`.

The simplest way to check what a variable contains is to write its name, and then
...
In the video we point at a variable name and value in the **Environment** pane.
...
1. Run the chunk under the `Exercise 1` heading in the workbook.
...
1. Create a new chunk below the `Exercise 2` heading in your workbook (`session-2-workbook.rmd`).
...
We often need to link a few functions together which modify data before analysing it.
...
In session 1, you used `head(fuel)` to show just the first few rows of the `fuel` dataset. Using a pipe, we can instead "pipe" the `fuel` dataset into `head()`:
...
In the video we highlight the visual reminder that the `tidyverse` package needs loading first
...
Remember that it is the final result of the functions on the right hand side of the `<-` that are assigned to the variable.
...
In the video we click on the `fuel_head` icon.
...
development_europe <- development %>% 
...
The value `"Kenya"` was in quotes because the country column is a factor (a column type R uses to store categories, like country names), not numeric,, and we need to
...
In the video we change `>` to `<` and run the code.
:::
...
Importantly, it also stores this data in a *new variable*, called `development_europe`.

We can look at this new subset of the data by typing and running its name, or by looking in the **Environment** pane.
...
Filter the `development` dataset to show the rows where a country's population was greater than 100 million.
(Each row is one country in one year, so some countries appear more than once.)
...
Show the rows where a country's population was greater than 100 million and life expectancy was greater than 70.
...
Separate each summary function with a comma:
...
Remember, the output from `summarise()` (in this case `fuel_summary`) is a data frame that you could do more processing
...
- RStudio has features to help you spot errors
...
The more you practice, the fewer mistakes you will make and the easier you will find it to spot errors which creep into
your code.

RStudio has some features which can help you spot errors.

Here's a chunk of code with various errors which we'll correct one by one.

:::{.direction}

In the video we open `broken.rmd`
...
In the video we run the code to show that it doesn't work.

:::

It's best to fix errors that produce the yellow triangle before going any further.

If you hover your pointer over the triangle, you will see a list of errors which RStudio has detected in the code.

:::{.direction}

In the video we hover over yellow triangle to show two warnings.

:::

The language used in these suggestions can be a bit confusing, but you can probably work out that RStudio is warning you that there's `no symbol named 'liibrary' in scope`, because of a simple typing error in the `library()` function.
...
1. Fix the error in the function name
1. Point out that the yellow triangle has gone
...
1. Fix the error in the package name
1. Execute the line with the `library()` function.
1. Point out the green bar.
...
Another symbol that RStudio uses to tell you that your code contains an error is the red circle with a white cross.
...
RStudio doesn't always correctly identify the correct way to resolve an error.
...
Notice what happens when we fix this error.

<!-- :::{.direction}

In the video we add the missing closing parenthesis.
...
In the video we hover the cursor over blue 'i'.

::: -->

For example, it might suggest adding spaces around operators like `%>%` or `==`, which makes the code easier to read.

### Errors which RStudio can't detect

Although your code may run, it may still not do what you expect.
...
1. Select the RStudio menu option `Session > Restart R`
...
(In the last plot, `colour=continent` inside `aes()` colours each point according to its continent; we cover this properly in a later session.)
...
This output uses the `fuel` dataset to show vehicles with 8 cylinders and an engine size greater than 5 litres (`engine_size` is measured in cc, so that's 5000cc).
...
The mean petal width of the `versicolor` species of iris is `r fitb(correct, tol=.01)`cm.
