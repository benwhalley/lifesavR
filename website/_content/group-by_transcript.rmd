In this video we'll use the `funimagery` dataset, which is part of the `psydata` package. This contains data from a
randomised controlled trial run in Plymouth [@solbrig2019functional].
...
This plot is helpful, and makes the differences really clear. But what if we want the actual numbers in a table, or to
...
Using `group_by()` before `summarise()` splits up the data, in thise case depending on which `intervention` the
...
but both ment and women lost more weight after FIT relative to MI.
