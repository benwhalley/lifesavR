...
Now we take the next step to quantify how *sure* we are that the patterns are real, using the *Bayes Factor*.
...
- [Interpreting Bayes Factors](#bayesinterpreting)

# Is this pattern a fluke?{#fluke}

```{r, echo=F}
video_data <- list(identifier = "bayes-fluke", yt_id="gefYqIPIjJQ")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "bayes-ttest", yt_id="BRKrSC5AjKg")
# makermds(video_data)
```
...
```{r, echo=F}
video_data <- list(identifier = "bayes-corr", yt_id="kUqabfExu2Q")
# makermds(video_data)
```
