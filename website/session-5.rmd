...
Create a series of revision exercises to scatter through subsequent sessions...
...
1. Use `summarise()` to calculate each group's weight at baseline
1. Use `ttestBF()` with a formula to calculate the Bayes Factor for the difference in these weights
...
- A Bayes Factor of $< \frac{1}{3}$ is evidence *against* the effect being real
- If the Bayes Factor is $> \frac{1}{3}$ but $< 3$ then we say the result is ***inconclusive***.
