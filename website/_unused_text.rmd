...
- filter with !is.na(outcome) before using count()

- when using correlations (pairwise or row-wise)

- filter(complete.cases(.)) and filter(complete.cases(select(., var1, var2)))
...
```{r}
funimagery %>%
ggplot(aes(kg1,, colour=intervention)) +
geom_density()
```
