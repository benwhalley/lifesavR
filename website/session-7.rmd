...
fuel %>% ggplot(aes(weight, mpg, colour=factor(cyl))) + 
  geom_point() + 
  geom_smooth(aes(group=1), colour="black", linewidth=.8, method="lm", se=F) + 
  geom_smooth(method="lm", se=F,  linewidth=1.2 ) + theme_minimal() + scale_colour_discrete("cylinders")
...
) %>% ggplot(aes(.resid, colour=m)) + geom_density() + scale_colour_discrete("Hypothesis")
...
In session 6 we saw how to [define the outcome and predictor variables for a simple
...
similar. Imagine we have a car with 4 cylinders, which weighs 1500kg (so `weight`=1500).

Our prediction is:

$intercept + (b_{cyl} \times 4) + (b_{weight} \times 1500)$

where $b_{cyl}$ and $b_{weight}$ are the coefficients for `cyl` and `weight`.

::: exercise
So, in this case, our prediction would be:
`r fitb(predict(modelB, newdata=tibble(weight=1500, cyl=4)), tol=1)`
...
So the calculation is $39 + 1500 \times -.007 + 4 \times -1.5 = `r 39 + 1500 * -.007 + 4 * -1.5`$
...
augment(modelB, newdata=tibble(weight=1500, cyl=4))
```

`augment` predicts `r predict(modelB, newdata=tibble(weight=1500, cyl=4)) %>% round(1)`, but this is because it did not round the coefficients before doing the
...
Why did it shrink towards zero?
...
So, in this example, *once we have adjusted for the number of cylinders*, the *adjusted* (or
*partial*) effect of weight is reduced (although not eliminated).
...
ii. Separately for cars with 4, 6 and 8 cylinders.
...
data*](session-6.html#residuals). *Larger residuals mean bigger errors in prediction*, and smaller
errors are better.
...
To calculate $R^2$ we use the `glance` function on our saved model result:

```{r}
glance(modelA)
```

***Explanation of the code***: We use the `glance` function, providing our saved result for `modelA` as the
input.

***Explanation of the output***: `glance` produces a data frame as a result, containing columns for
`r.squared`, `adj.r.squared`, and some other statistics which we don't need for now. The number to
focus on here is `adj.r.squared`, which is 0.75.
...
Our final task is to *quantify how much evidence* we have that model B is better than model A. For
...
how many times more likely the data we collected are under one hypothesis than under an alternative
hypothesis.
...
Next we re-run the models using the `lmBF` function. The only change is to replace `lm` with
`lmBF`.
...
```{r, echo=F, include=F}
bfBA <- (mB/mA) %>% as.data.frame() %>% mutate(across(where(is.numeric), ~round(.x, 2)))
```
...
hypothesis](session-5.html#interpretbayesfactors1).

In this example the Bayes Factor is `r bfBA$bf`, so we have strong evidence in favour of model B,
compared with model A.

Put differently, the data we collected are 22 times more likely under model B than under model A.
:::
:::
...
- BFs > 3 are *moderate* evidence *for* a hypothesis, and 
...
Several authors have written guidelines on how to interpret Bayes Factors in this way. They
provide labels for Bayes Factors of various sizes. This is more useful than a simple "line in the
sand", especially when comparing multiple studies. The labels below are from @lee2013bayesian,
who adapted them from Jeffreys (1961). @kass1995bayes proposed a similar scheme with slightly
different cut-offs.
...
'Anecdotal' evidence (a BF between 1 and 3) is what we called *inconclusive* in
[session 5](session-5.html#interpretbayesfactors1).



**Bayes Factors can also be smaller than 1. These indicate support for the *other* hypothesis**:
the one on the bottom of the division (when we test an effect against 'no effect', this is the
null hypothesis). The same labels apply in reverse: for example, a BF of 0.05 is strong evidence for
the other hypothesis (because $1/0.05 = 20$), but a BF between $\tfrac{1}{3}$ and 1 is still inconclusive.
If a BF is less than 1 we often find it helpful to reverse the order of division so that the BF is
larger than 1, and state explicitly in the text which hypothesis the BF supports.
...
and the Bayes Factor for Model B is 560.


- Which model do the data support? `r mcq(c("A", answer="B", "Inconclusive"))`
- The BF in favour of model B is: `r mcq(c("22", "560", answer="25.5", "0.04", "3.6"))`
- The BF in favour of model A is (to 2 decimal places): `r fitb(22/560, tol=.01)`


`r hide("See explanation")`


- For the BF in favour of model A, we calculate $BF_{A} / BF_{B}$ = 22/560 = 0.04

- For the BF in favour of model B, we calculate $BF_{B} / BF_{A}$ = 560/22 = 25.5


Note that $1 / 0.039 = 25.5$. We can use this trick ([taking the reciprocal](https://www.cliffsnotes.com/cliffsnotes/subjects/math/in-math-what-does--i-reciprocal-i-mean)) to 'reverse' any Bayes Factor that is < 1 
...
$R^2_{adjusted}$ was .75 for model A, and .82 for model B, which means both models were quite good.
...
that — although both were quite a good fit to the data — the data were 22 times more likely under model B
than under model A.
...
Write an answer to each of these questions in the `Check your knowledge` section of your workbook.
...
bfsize2 <- c(
  ...
  "some evidence for the hypothesis tested",
  "strong evidence for the hypothesis tested",
  answer="strong evidence for the other hypothesis (e.g. the null)"
)

```

1.  We prefer models with `r mcq(smalllargeresid)` residuals

1.  The $R^2$ statistic ranges from: `r mcq(rsquarerange)`

1.  It's better for a model to have a `r mcq(rsqlarsm)` $R^2$ value

1.  If we calculate the Bayes Factor for `modelA / modelB` and the result is \> 1, this provides
    evidence that `r mcq(bfq1)`

1.  A Bayes Factor of 25 means we have `r mcq(bfsize)` evidence

1.  A Bayes Factor of 0.05 means we have `r mcq(bfsize2)`
...
They ask you to run additional analyses to build confidence using R.

::: exercise
#### Task 1: Interpret these plots

```{r, fig.width=7, fig.height=3, echo=F}
p1 <- studyhabits %>% ggplot(aes(work_hours, grade)) + geom_point() + geom_smooth(method="lm", se=F)
p2 <- studyhabits %>% ggplot(aes(work_consistently, grade)) + geom_point() + geom_smooth(method="lm", se=F)
p3 <- studyhabits %>% ggplot(aes(work_consistently, work_hours)) + geom_point() + geom_smooth(method="lm", se=F)
...
-   The code to calculate the Bayes Factor in favour of Model B is: `r mcq(formodlB)`
...
borabetter <- c(
  "Model B is much better than model A",
...
```{r, echo=F}
abchoice <- c(
  "The data are only 0.1% more likely under model A than under model B",
  "The data are only 0.1% more likely under model B than under model A",
  "The data are 91% more likely under model A than under model B",
  "The data are 91% more likely under model B than under model A",
  "The data are 91 times more likely under model A than under model B",
  answer="The data are 91 times more likely under model B than under model A"
)
```
...
The plots below show the distribution of and relationship between three columns in the `happy`
dataset, in the `psydata` package.

These data are a simplified version of the [Gallup World Happiness
Report](https://worldhappiness.report/ed/2021) (see the report for more details on the questions asked).

The columns shown are the national average of responses to three questions:

-   `happiness`: a measure of subjective well-being, rated on the Cantril ladder from 0 (worst
...
::: exercise
#### Task 7: National happiness

Using the `happy` data:

-   Write code to run linear models to predict `happiness` using the `social_support` and
    `freedom_to_make_life_choices` columns.
