...
development %>%
  ...
  xlab("") + ylab("") +theme(axis.title.x=element_blank(),
        ...
        axis.ticks.y=element_blank()) +guides(colour="none") + theme(
...
In this workshop we learn how to use categorical variables in boxplots, to add colour to scatterplots, and
...
send this data to the next function:
...
-   Emphasise the importance of breaking code down and running sub-steps separately
:::
...
1. Open `session-3-workbook.rmd` (in the `exercises` folder) using the **Files** pane. This is the workbook you will be using in this session.
...
# make a scatter plot with two continuous axes
# both weight and mpg are numeric variables
# so this works well
fuel %>%
  ggplot(aes(weight, mpg)) +
    geom_point()   

# try and make a boxplot with `cyl` as the x-axis
...
-   *variables in a study design*. An example here might be an experimental group allocation, or an
...
**In this short course we use *variable* to mean either an experimental/theoretical variable or an R
container.**
...
-   *Numeric* columns can be either *integer* or *double*. Double is computer speak for
...
You can see which data type is used to store a column when using the `glimpse()` function
introduced in Session 1 (see [here](session-1.html#datasets)).

```{r}
iris %>% glimpse()
```

In the output you can see the column names listed on the left, followed by text in angle brackets,
e.g.: `<dbl>` which is the abbreviated name of data type.

In this built-in dataset, most of the data is numeric (`dbl`), but the `Species` column is
...
The warning message includes `Continuous x aesthetic -- did you forget aes(group=...)?`. This is a clue
...
*continuous scale* on the $x$-axis.

Only one box is drawn at the midpoint of all the values of `cyl`; because `cyl` ranges from 4 to 8
...
should convert `cyl` to a `factor` explicitly. Then our plot will work properly.

We can use the function `factor(cyl)` to tell R that the $x$-axis is a factor:
...
The only change was to replace `cyl` with `factor(cyl)`. This tells R to convert the column `cyl`
...
Use `glimpse()` to check the data types of the `mpg` and the `diamonds` datasets. (Hint: in the `glimpse()` output,
`<int>` means integer and `<dbl>` means double.)

- The data type of the 4th column in the `mpg` data is `r mcq(c("double", "factor", "character", answer=typeof(mpg[[4]])))`
- The data type of the 5th column in the `diamonds` data is `r mcq(c("date","character", "factor", answer=typeof(diamonds[[5]])))`
...
Remember that you will need to *change* the type of the `gear` column for this to work.
...
-   The first measurement (`kg1`) was taken at *baseline*, when participants joined the study
...
The `weight_lost_end_trt` column is the *difference* between `kg1` and `kg2` (it's calculated as
`kg2 - kg1`). This means that **weight lost is a negative number**: a participant who went from
100kg to 95kg has a value of -5. So *lower* (more negative) values mean *more* weight was lost.

We have already seen that a boxplot can compare scores between categories:
...
This is a bit repetitive, and would be worse if there were lots of categories. Imagine doing something
...
Using `group_by()` before `summarise()` splits up the data, in this case depending on which
...
In RStudio you will also see a message like `` `summarise()` has grouped output by 'gender'. You can
override using the `.groups` argument. `` This is just information, not an error --- you can ignore it.
...
Replace the `?` in the code below to calculate the `median` weight lost for men and women undergoing FIT and MI in the
`funimagery` dataset.
...
`r hide('show answer')`
The best clarity, `IF` (shown in yellow). For the same increase in carat, price rises most steeply
for `IF` diamonds and least steeply for the lowest clarity, `I1`.
`r unhide()`
...
Interpret your plot: Which type of car (auto or manual) sees the strongest relationship between engine size and fuel economy?

`r hide('show answer')`
Automatic cars (`automatic` is `TRUE`). For both types, cars with bigger engines do fewer miles per gallon, but mpg
drops more steeply as engine size increases for automatic cars.
`r unhide()`
:::
