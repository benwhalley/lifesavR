...
R is a text-based language. You interact with it by typing code and running (also called 'executing') it.
...
Now we have 3 statements: `2 + 2 + 8 + 16`, `42 * 42` (the star means multiply), and a third which
adds `2 + 4 + 8 + 16` but is spread over several lines.
We can run any of the statements in the same way: by putting the cursor anywhere on the statement
...
**Source** pane because statements written in the R language are often referred to as 'R code', which is shorthand for 'R source code'. The **source** pane allows you to write R code and explore your data.
...
1. Run the line of code you have written.
...
Running `library(tidyverse)` loads some additional functions and data which will allow us to make a scatterplot.
...
In the video we demonstrate an error when running plot code if `tidyverse` hasn't been loaded
...
1. Load `tidyverse`
1. Re-run the example plot to show that it now works.
...
until you've run the code: `library(tidyverse)`. And the data in `psydata` is not available until
after you load that package.


For example, if you tried to produce a scatterplot before loading `tidyverse` you'd see an error like this in the
console pane:

<pre style="color:red">
Error in diamonds %>% ggplot(aes(carat, price)) :
...
### Datasets are like spreadsheets. They have:
...
- Data from packages like `psydata` don't appear in the **Environment** pane; it only lists data you have created yourself
...
In R, datasets contain values which are organised into columns and rows.

One distinction to make though is that by *dataset* we mean data that has
been loaded into R--- data *files* are a different thing.
...
In the video we show `library(psydata)` and then the `fuel` dataset.
...
There are two ways we recommend you inspect and check data you are using.
...
`glimpse` shows a list of all the columns in the dataset, the type of data stored in each column, and as many observations (datapoints) as will fit on a single line.
...
Second, these data relate to concrete, easy to understand phenomena (e.g. weight, length, number of gears).
...
1. Open your workbook for this workshop (called `session-1-workbook.rmd`).
...
In your workbook (`session-1-workbook.rmd`):
...
Use the output to answer the following question. After entering your answer, click outside the box. The border will turn blue when the answer is correct.
...
In the video we select the "pipe" with the cursor to highlight it.
...
2. Emphasise the axes are there, but no data shown yet.
3. Rerun all code to plot  points for each row in the data.
...
In the video we select the x-axis and y-axis in turn when describing them.
...
- A ***statement*** is one or more lines of code which R knows are linked
...
- This chapter: <https://clauswilke.com/dataviz/visualizing-associations.html> shows many examples of plots which display relationships between variables (including scatterplots) which would extend the material covered above.
