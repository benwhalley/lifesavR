...
> '**_fitted_**' to a sample it becomes a tool to make **_predictions_** for
...
> predictors and outcomes are. But the meaning of coefficients depends entirely
> on the **_design_** of our study, and the **_assumptions_** we are prepared
> to make. Causal diagrams can help us decide how
...
- [Revision for scatter plots](#scatterplotrevision)
- [Adding straight lines to scatter plots](#bestfitlines)
- [Explanation of the regression equation (describing a straight line in numbers)](#equationforline)
- [The regression line as a *prediction*](#predictions)
- [Residuals as the *error in prediction*](#residuals)
- [Three *assumptions* regression makes](#assumptions3)
...
Open the data and list the columns in the `mentalh` dataset:
...
## Showing relationships {#scatterplotrevision}
...
- remember to use the `aes` function to select which column appears on each axis
...
## Adding best fit lines with `geom_smooth()` {#bestfitlines}
...
- The *predictor* is what we use to predict the outcome variable (e.g., average hours of screen time per day)

- The outcome must be a *continuous* variable. In this session the predictor is continuous too (later you'll see that predictors can also be categorical)


In this session we stick to drawing **straight lines** 
because they represent the simplest-possible relationship.

The best way to 'see' a simple linear regression is to use the `geom_smooth()` function. 
...
:::{#O3G3m-iZKbg-explanation .tab-pane .fade}
...
1. Recreate the plot in the video with `screen_time` as the predictor, `anxiety_score` as the outcome, and with the line of best fit (using `geom_smooth(method="lm")`)           

2. Make two versions: one with `method="lm"`, and one without. The lines look different — but which do you think is the "best"? How would we decide? (We will return to this topic later)
...
# Equation for a straight line {#equationforline}
...
1. The expected value is `r egdata.model.coefs[1] %>% sprintf("%.2f",.)`. This is the intercept.
...
- The statement on line 2 tells R to use the `lm` function. 
...
-   The 'Call' we made (i.e. what inputs we gave, so we can remember how we did it later on; these are the first lines of the output)
...
2. Make a new data frame, with the values of the predictor we will predict-for
3. Use the `augment` function to make the predictions
...
# make predictions for our new data frame (i.e. for screentime = 10)
...
:::{#v_S39Ozem4JY-explanation .tab-pane .fade}
...
$\rm{Anxiety\ score} = `r s1coef[1]` + (`r s1coef[2]` \times \rm{Screen\ time})$
...
$\rm{Anxiety\ score} =  `r s1coef[1]` + (`r s1coef[2]` \times 10) = `r s1coef[1] + s1coef[2]*10`$
...
We can check this is the same model by using the name we stored it under as an R statement:

```{r}
simple1
```

### Step 2: Make a new data frame, with the values of the predictor we will predict-for
...
**Explanation of the output**: We have a new data frame, called `new_data`, which contains 1 row and 1 column (`screen_time`).


### Step 3. Use the `augment` function to make the predictions.
...
Then we can use `augment` with the stored model (`simple1`), and the new data frame (`new_data`): 

```{r}
augment(simple1, newdata=new_data)
```

**Explanation of the output**: The `augment` added new columns to the `new_data` data frame. The predicted `anxiety_score` is in the `.fitted` column and is 6.91.
...
2. What is the expected anxiety score for a person who watched a screen for 5 hours? `r fitb(s1coef[1] + s1coef[2]*5, tol=.1)`
...
```{r, echo=F}
augment(simple1, newdata = tibble(screen_time = c(2, 5))) %>% 
  pull(.fitted) %>% round(3) %>% 
  pandoc.list()
```
...
3. Make a new data frame so that you can predict `anxiety_score` for someone who watched screens for 8 hours
4. Use `augment` to make the predictions

```{r, include=F, echo=F}
person8hrs <- augment(simple1, newdata=tibble(screen_time=8)) %>% pull(.fitted) %>% first()
```

The expected anxiety score for someone who watched screens for 8 hours is: `r fitb(person8hrs, tol=.1)`
...
:::{#residuals-explanation .tab-pane .fade}
...
```{r}
augment(simple1) %>%  # augment returns a data frame of predictions and residuals
...
- We used the `head` function to show only the 
first few rows and 
...
```{r, include=F}
residmcq <- c(
  "data that wasn't used to fit the model",
  answer = "distance between observed outcomes and model predictions",
  "prediction for new data",
  "distance between two points on the regression line"
)
...
- The residual is the `r mcq(residmcq)`
- Residuals can be calculated with which R function? `r mcq(whichrresdid)`
- Residuals will be on the same scale as the: `r mcq(residwhichscale)`
...
```{r, include=F}
mentalh$anxiety_score[3]
mentalh$screen_time[3]
fitted(simple1)[3]
mentalh$anxiety_score[3] - predict(simple1)[3]
```

Consider the person in the **third row** of the `mentalh` dataset we used to fit the model we saved as `simple1`:

- What is their anxiety score? `r fitb(mentalh$anxiety_score[3], tol=.1)`
- How much screen time did they watch? `r fitb(mentalh$screen_time[3], tol=.1)`
- What is their **predicted** anxiety score? `r fitb(predict(simple1)[3], tol=.1)`
- What is the **residual** for this prediction? `r fitb(resid(simple1)[3], tol=.1)`
...
:::{#checkingassumptions-explanation .tab-pane .fade}
...
To check this we use another scatterplot with the *fitted values* (the predictions, i.e. the points on the line) on the $x$-axis, and the *residuals* on the $y$-axis.
...
Write an answer to each of these questions in the `Check your knowledge` section of your workbook. The answers will be revealed in Session 7.

1. What is an intercept? `r mcq(whatintercept)`
1. What is the slope? `r mcq(whatslope)`
1. What is a residual? `r mcq(whatresid)`
1. Which function fits a regression model? `r mcq(whatfuncmodel)`
1. Which function calculates residuals and predictions from a saved `lm` model? `r mcq(whatfuncrespred)`
1. What are the three assumptions of regression introduced in this session?  (Make a note of your answers, then [check them here](#assumptions3))
...
2. Inferences we make from the model (e.g. Bayes Factors, which we will use with regression models in the next session) assume that the residuals are normally distributed. If this is not the case, then these statistics can be biased. Specifically, we may conclude that we have more evidence for an effect than is really the case. This will lead us to draw the wrong conclusions from our data and, potentially, make bad decisions.
...
combineddiagplots <- function(f, thedata){
  ...
  thedata <- thedata %>% slice_sample(n=min(100, nrows))
  themodel <- lm(f, data=thedata)
  x <- as.list(f)[[3]] %>% as.character()
  y <- as.list(f)[[2]] %>% as.character()

  modeldata <- themodel$model
  # model.frame puts the outcome in the first column
  modeldata$x =modeldata[,2]
  modeldata$y =modeldata[,1]
  aa <- modeldata %>% 
    ggplot(aes(x,y)) + geom_jitter(alpha=.5) + 
    geom_smooth(se=F) +
    geom_smooth(method="lm", se=F, color="red", linewidth=.4) +
    labs(x="Predictor", y="Outcome") + ggtitle("Scatterplot")
  a <- augment(themodel, newdata=thedata) %>%
    ggplot(aes(.resid)) +
    geom_histogram(bins=25)+
     ggtitle("Distribution")
  b <- augment(themodel, newdata=thedata) %>%
    ggplot(aes(.fitted, .resid)) +
    geom_hline(yintercept = 0, color="red",  linewidth=.4) +
    geom_jitter(alpha=.5) +
...
I have added red lines showing the *expected* trend of the data, if the assumptions had been met
...
The outcome of our model is the average life expectancy of each country in years.
...
The outcome of our model is the price of a diamond in US dollars. The predictor is the size of
...
not normally-distributed, and the residuals get larger as the prediction increases.
...
The predictor is the weight of the car (measured in 1000s of pounds).
...
- The relationship looks linear: `r mcq(c("True", "False", answer="Hard to tell"))`
...
*Interpretation*: In this model it is hard to tell whether the true relationship is linear, in part because we only
have 32 datapoints in the sample. Likewise, it's hard to tell from the histogram if the residuals are normal.
However, the fitted vs residual plot shows a clear U-shaped trend: residuals are mostly positive at low and high
fitted values and mostly negative in the middle. The residuals are not consistent across the range of fitted values
(this also suggests the true relationship may be slightly curved), so it would be worth exploring further (see next section).
...
The outcome of our model is the length of petals (in cm). 
The predictor is the width of the petals (cm).
...
Many students will not have time for this additional material. However it is provided as extended learning for those
who may choose to use regression or related techniques in stage 4.



:::

## Understanding and fixing violations of assumptions {#fixingviolations}




In an [example above](#violationexample) we saw that a model predicting life expectancy from GDP per-person led us to violate some assumptions of regression. There are many different techniques which can be used to work around this kind of problem. Here we illustrate just one.
...
development %>% 
  ggplot(aes(gdp_per_capita, life_expectancy)) + geom_point(alpha=.2) + geom_smooth(method="lm", se=F)
```
...
Incomes are _not_ normally distributed. 
In fact, they are very [*right skewed*](https://www.statology.org/left-skewed-vs-right-skewed/): most GDP values are quite low, but there is a long tail on the right hand side (a few countries have very large GDPs relative to the median).  

This is quite typical of incomes data. At the level of countries, and for individuals, wealth is very unequally distributed. 
Some people are really [a _lot_](https://bezoscalculator.com) richer than the average.


### Transforming the predictor

One option in these cases is to *transform* the predictor. Predictors don't need to be normally
distributed, but a very skewed predictor like this often has a curved relationship with the outcome.

For example, if we plot the [logarithm](https://www.khanacademy.org/math/algebra2/x2ec2f6f830c9fb89:logs/x2ec2f6f830c9fb89:log-intro/v/logarithms) of GDP it becomes (a bit more) normally distributed. 
...
```{r, echo=F, fig.cap="Diagnostics plots from a regression model using a log-transformed predictor.", fig.height=2.5}
...
- Make a scatterplot of the `mpg` (miles per gallon) and `hp` (horsepower) columns
- Add a fitted line using `geom_smooth`
- Use a `log()` transform on the `hp` column and make the plot again
