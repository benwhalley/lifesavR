...
```{r, echo=F}
video_data <- list(identifier = "describe-centrantend", yt_id="WcfyjFCdnWk")
# makermds(video_data)
```
...
funimagery.edited <- funimagery %>%
  mutate(weight_lost_time_two = kg2 - kg1)

funimagery.edited %>%
  ggplot(aes(intervention, weight_lost_time_two)) +
  geom_boxplot()
```
...
```{r, echo=F}
video_data <- list(identifier = "describe-frequency", yt_id = "2zH8QyrcMqU")
# makermds(video_data)
```
