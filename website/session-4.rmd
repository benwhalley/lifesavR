...
- The first step is to describe and visualise patterns in our data
...
However, more important than any specific technique in R, is this idea that we ***collect data to answer questions*** (not just for its own sake!).
...
If we have a single continuous variable — that is, one stored in a numeric column in R —  then can describe a few things about it, including:
...
There is also a column in this dataset called `weight_lost_end_trt`,
...
As with `weight_lost_end_trt` in session 3, this means **weight lost is a negative number**: lower (more negative)
values mean more weight was lost.
...
- run the code above and show the result.
...
To do this we *assign* the result of `mutate()` to a new variable (the 'container' type of variable).
...
In the video we show how a new variable has been created in the ***Environment*** window
...
```{r}
# boxplot of weight lost at end of follow-up using new column
funimagery.edited %>%
...
1. Open `session-4-workbook.rmd` (in the `exercises` folder) using the **Files** pane. This is the workbook you will be using in this session.
...
```{r, echo=F, include=F}
aesmcqq <- c("aes(Sepal.Width, Species)", "aes('Species', 'Sepal.Width')", "aes((species, sepal.width))", answer="aes(Species, Sepal.Width)")
```
...
# making a plot
# stat_count() counts the rows in each category and draws a bar for each
earnings %>%
  ggplot(aes(gender)) +
  stat_count()
```
...
The earnings data contains columns called `gender` and `job`, which give us
...
is again from the tidyverse package.

:::{.direction}
Load the tidyverse now
:::
...
What's neat about `count()` is that we can add more than one column at once, and we get a count for each sub-grouping that is formed. For example:
...
- With 2 continuous variables we can also describe a *relationships* between them:
  ...
  - Correlations (using `correlate()` from the `corrr` package)
- Use `select()` to choose the columns to correlate
...
The scatterplot shows there is a ***strong, positive relationship*** between participants' weights at
...
The `select()` function (from the tidyverse) keeps only the columns we name and drops the rest. For example, this
gives us a dataset with just the three weight columns:
...
- show how R spits out three columns of data below the chunk, rather than the whole dataset.

:::


```{r, message=F}
# select weight at baseline and follow-up and correlate them
funimagery %>%
  select(kg1, kg2, kg3) %>%
  correlate()
```
...
It has one column for each of the columns we selected, plus an extra column called `term` for the column
being correlated.
...
In the video we show and talk through some of the values in the table at this point.
...
- Scatterplots and correlations help describe relationships
...
1. What functions in R describe the central tendency of a variable?
1. What type of column can we use with `mean()`?
1. How might you describe patterns in a categorical variable?
1. What function did we use to create a new column in the `funimagery` dataset?
1. In the `funimagery` data, what was the mean change in weight (`kg3 - kg2`) for FIT participants AFTER the end of treatment? (Remember that a negative number means weight was lost.)
1. What type of plot describes the relationship between two variables?
1. What function do we use to make a plot that shows a relationship between two variables in `ggplot()`?
